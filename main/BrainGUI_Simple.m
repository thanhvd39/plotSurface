...
% Fixed version that actually works without errors
    ...
    function createSimpleGUI()
        ...
        min_edit = uicontrol('Style', 'edit', ...
                           'String', '-1', ...
                           'Position', [140, 320, 60, 25]);
        
        uicontrol('Style', 'text', ...
                 'String', 'Max Value:', ...
                 'Position', [220, 320, 80, 20], ...
                 'BackgroundColor', get(fig, 'Color'), ...
                 'HorizontalAlignment', 'left');
        
        max_edit = uicontrol('Style', 'edit', ...
                           'String', '1', ...
                           'Position', [310, 320, 60, 25]);
        
        % Auto range checkbox
        auto_range_check = uicontrol('Style', 'checkbox', ...
                                   'String', 'Auto Range', ...
                                   'Position', [400, 320, 100, 25], ...
                                   'Value', 1, ...
                                   'BackgroundColor', get(fig, 'Color'), ...
        ...
        uicontrol('Style', 'pushbutton', ...
                 'String', 'Save Figures', ...
                 'Position', [50, 190, 120, 30], ...
                 'Callback', @saveFigures);
        
        uicontrol('Style', 'pushbutton', ...
                 'String', 'Clear All', ...
                 'Position', [180, 190, 120, 30], ...
                 'Callback', @clearAll);
    ...
    function generateTestData(~, ~)
        try
            ...
            brain_data = randn(200, 1);
            
            % Add some structure
            brain_data(1:50) = brain_data(1:50) + 1.5;      % Strong positive
            brain_data(51:100) = brain_data(51:100) - 1.2;  % Strong negative
            brain_data(101:150) = brain_data(101:150) * 0.3; % Weak values
            
            current_file = 'Generated Test Data';
            set(file_display, 'String', current_file);
            updateInfoDisplay();
            ...
            msgbox('Test data generated successfully!', 'Success');
            
        catch ME
            msgbox(['Error generating test data: ' ME.message], 'Error', 'error');
        end
    end
    ...
    function updateInfoDisplay()
        if isempty(brain_data)
            ...
            info_str = sprintf(['Atlas: %s\n' ...
                               ...
                               'Mean: %.4f ± %.4f\n' ...
                               'Non-zero: %d/%d'], ...
                               atlas_name, current_regions, region_match, ...
                               size(brain_data, 1), size(brain_data, 2), ...
    ...
    function plotCsvDirect(~, ~)
        ...
        try
            ...
            elseif exist('plot_brain_with_atlas', 'file')
                msgbox(sprintf('Generating brain surface plot using %s atlas...', current_atlas), 'Processing');
                plot_brain_with_atlas(brain_data, current_atlas, image_name);
                msgbox('Brain surface plot generated successfully!', 'Success');
            elseif exist('plot_brain_gui_safe', 'file')
                msgbox('Generating brain surface plot...', 'Processing');
                plot_brain_gui_safe(csv_file, image_name);
                msgbox('Brain surface plot generated successfully!', 'Success');
            elseif exist('plot_your_csv_data', 'file')
                msgbox('Generating brain surface plot...', 'Processing');
                plot_your_csv_data(csv_file, image_name);
                msgbox('Brain surface plot generated successfully!', 'Success');
            else
                msgbox('Brain plotting functions not found!', 'Error', 'error');
            end
    ...
    function saveFigures(~, ~)
        try
            % Find all figures except the GUI
            ...
            for i = 1:length(all_figs)
                ...
                if ~contains(fig_name, 'Brain Surface Plot - Simple GUI')
                    plot_figs = [plot_figs; all_figs(i)];
                end
            end
            
            if isempty(plot_figs)
                msgbox('No plot figures to save!', 'No Figures', 'warn');
                return;
            end
            ...
            for i = 1:length(plot_figs)
                filename = fullfile(output_dir, sprintf('brain_plot_%d_%s.png', i, timestamp));
                print(plot_figs(i), filename, '-dpng', '-r300');
            end
            
            msgbox(sprintf('Saved %d figures to output directory', length(plot_figs)), 'Success');
            
        catch ME
            msgbox(['Error saving figures: ' ME.message], 'Error', 'error');
        end
    end
